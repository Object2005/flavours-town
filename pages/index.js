...
export default function Home() {
  ...
  const handleAdminToggle = () => {
    if (isAdmin) setIsAdmin(false);
      ...
      else alert("Only Aashray can access this!");
    }
  };
  ...
  const handlePayment = (method) => {
    ...
    else window.location.href = `upi://pay?pa=9877474778@paytm&pn=Aashray%20Narang&am=${total}&cu=INR&tn=Order%20at%20Flavours%20Town`;
  };

  return (
    <div className="bg-[#fcfcfd] min-h-screen pb-72 font-sans selection:bg-orange-500/30">
      <Head>
        <title>Flavour's Town | Elite App by Aashray</title>
      ...
      <header className="fixed top-0 w-full z-[100] px-4 py-4 backdrop-blur-2xl bg-white/80 border-b border-gray-100 shadow-sm">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
             <div className="bg-gradient-to-br from-orange-500 to-red-600 h-11 w-11 rounded-[14px] flex items-center justify-center text-white font-black text-xl shadow-lg ring-2 ring-white">FT</div>
             <div>
                <h1 className="text-lg font-black tracking-tighter text-gray-900 leading-none">FLAVOUR'S<span className="text-orange-600">TOWN</span></h1>
                <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">Swaad Jo Yaad Rahe</p>
             </div>
          </div>
          <button onClick={handleAdminToggle} className={`w-10 h-10 rounded-full flex items-center justify-center shadow-inner ${isAdmin ? 'bg-orange-600 text-white' : 'bg-gray-50 text-gray-400'}`}>⚙️</button>
        </div>
      </header>
      ...
      <nav className="pt-28 pb-4 px-4 max-w-7xl mx-auto flex gap-2 overflow-x-auto no-scrollbar relative z-30">
        {["All", "Chaap", "Tikka", "Rolls", "Snacks", "Sweets"].map(cat => (
          <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-7 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shrink-0 ${activeCategory === cat ? 'bg-orange-600 text-white shadow-xl scale-105' : 'bg-white text-gray-400 border border-gray-100 shadow-sm'}`}>
            {cat}
          </button>
        ))}
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-4">
        {menu.filter(i => activeCategory === "All" || i.category === activeCategory).map(item => (
          <motion.div layout key={item.id} className={`bg-white rounded-[2.8rem] p-3 shadow-xl border border-white relative ${!item.inStock ? 'opacity-40 grayscale' : ''}`}>
            <div className="relative rounded-[2.2rem] overflow-hidden mb-4 h-44 shadow-lg">
              <img src={item.img} className="w-full h-full object-cover" />
            </div>
            <div className="text-center px-1">
              <h3 className="text-[12px] font-black mb-1 uppercase tracking-tighter h-8 flex items-center justify-center leading-tight">{item.name[lang]}</h3>
              <p className="text-orange-600 font-black text-xl italic mb-4 tracking-tighter">₹{item.price}</p>
              
              {!isAdmin ? (
                <div className="flex items-center justify-between bg-gray-50 p-1 rounded-2xl border border-gray-100">
                  <button onClick={() => setCart({...cart, [item.id]: Math.max(0, (cart[item.id] || 0) - 1)})} className="w-10 h-10 rounded-xl bg-white shadow-md font-black text-orange-600">-</button>
                  <span className="font-black text-sm">{cart[item.id] || 0}</span>
                  <button onClick={() => setCart({...cart, [item.id]: (cart[item.id] || 0) + 1})} disabled={!item.inStock} className="w-10 h-10 rounded-xl bg-orange-600 text-white shadow-lg">+</button>
                </div>
              ) : (
      ...
      <footer className="mt-24 px-6 py-20 bg-white border-t border-gray-100 text-center">
        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-300 mb-4">Coded & Designed by</p>
        <h2 className="text-3xl font-black italic text-gray-900 tracking-tighter mb-10">AASHRAY <span className="text-orange-600 underline decoration-gray-100 underline-offset-8">NARANG</span></h2>
        
        <div className="flex justify-center gap-6 mb-12">
          <a href="https://github.com/Object2005" target="_blank" className="p-4 bg-gray-50 rounded-2xl shadow-sm hover:scale-110 transition-transform">🐙</a>
          <a href="https://linkedin.com/in/aashray-narang" target="_blank" className="p-4 bg-gray-50 rounded-2xl shadow-sm hover:scale-110 transition-transform text-blue-600">💼</a>
          <a href="tel:+919877474778" className="p-4 bg-gray-50 rounded-2xl shadow-sm hover:scale-110 transition-transform text-green-600">📞</a>
          <a href="http://googleusercontent.com/maps.google.com/4" target="_blank" className="p-4 bg-gray-50 rounded-2xl shadow-sm hover:scale-110 transition-transform text-orange-600">📍</a>
        </div>
        <p className="text-[10px] text-gray-400 font-bold italic uppercase tracking-widest">Premium Food Tech • 2026</p>
      </footer>

      {/* --- ANIMATED ELITE CART BAR --- */}
      <AnimatePresence>
        {total > 0 && (
          <motion.div initial={{ y: 200 }} animate={{ y: 0 }} exit={{ y: 200 }} className="fixed bottom-10 left-4 right-4 z-[110] flex justify-center">
            <button onClick={() => setShowCheckout(true)} className="w-full max-w-md bg-orange-600 p-6 rounded-[3.5rem] shadow-[0_25px_60px_-15px_rgba(234,88,12,0.5)] flex justify-between items-center text-white ring-4 ring-white/30 active:scale-95 transition-all">
               <div className="flex items-center gap-4 italic">
                  <div className="bg-white/20 h-14 w-14 rounded-2xl flex items-center justify-center text-3xl">🛍️</div>
                  <div className="text-left"><p className="text-[9px] font-black uppercase opacity-70 mb-1">My Cart Total</p><p className="text-3xl font-black tracking-tighter text-white">₹{total}</p></div>
               </div>
        ...
        {showCheckout && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[200] flex items-center justify-center p-6">
            <div className="bg-white p-12 rounded-[4.5rem] max-w-sm w-full shadow-2xl text-center border-t-[12px] border-orange-600">
              <h2 className="text-xl font-black uppercase italic mb-2 tracking-tighter text-gray-800 tracking-[0.2em]">Final Bill</h2>
              ...
              <div className="flex flex-col gap-4">
                  <button onClick={() => handlePayment('WhatsApp')} className="w-full bg-[#25D366] text-white py-5 rounded-[2rem] font-black uppercase tracking-widest shadow-xl flex items-center justify-center gap-3 transition-transform active:scale-95">📱 WhatsApp Order</button>
                  <button onClick={() => handlePayment('UPI')} className="w-full bg-[#1A73E8] text-white py-5 rounded-[2rem] font-black uppercase tracking-widest shadow-xl flex flex-col items-center justify-center active:scale-95 transition-transform leading-none">
                    <span>💳 Pay via UPI App</span>
                    <span className="text-[8px] mt-1 opacity-70">(GPay, Paytm, PhonePe)</span>
                  </button>
