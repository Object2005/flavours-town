...
const CountdownTimer = ({ targetDate }) => {
  ...
  return <span className="font-mono text-orange-400 font-bold tracking-widest">{timeLeft}</span>;
};
...
const CameraScanner = ({ onClose }) => {
    ...
    useEffect(() => {
        ...
        const startCamera = async () => {
            try {
                ...
                setError("Camera blocked or not available. Please ensure permission is granted.");
            }
        };
        startCamera();
        return () => {
    ...
    return (
        <div className="fixed inset-0 z-[300] bg-black flex flex-col items-center justify-center p-4">
            <h2 className="text-white font-black text-xl mb-4 animate-pulse">SCAN CART QR CODE</h2>
            <div className="w-full max-w-sm aspect-square bg-zinc-900 rounded-3xl overflow-hidden border-4 border-orange-500 relative shadow-[0_0_50px_orange]">
                {error ? <p className="text-red-500 text-center p-10 font-bold">{error}</p> : <video ref={videoRef} autoPlay playsInline className="w-full h-full object-cover" />}
                <div className="absolute inset-0 border-[50px] border-black/50"></div>
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 border-2 border-white/50 rounded-lg animate-ping opacity-20"></div>
            </div>
            <p className="text-gray-400 text-xs mt-6 text-center max-w-xs">Point camera at the QR code on the counter to pay via UPI</p>
            <button onClick={onClose} className="mt-8 bg-white text-black px-8 py-3 rounded-full font-black uppercase text-sm">Close Camera</button>
        </div>
    );
};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const savedMenu = localStorage.getItem('ft_v14_menu');
    ...
    // 🚀 ROBUST SYNC LOOP
    const interval = setInterval(() => {
        const storedOrders = JSON.parse(localStorage.getItem('ft_v14_orders') || '[]');
        
        // ADMIN: Update Live List
        setLiveOrders(storedOrders); 

        // USER: Update My Order Status
        const myActive = storedOrders.find(o => o.id === sessionOrderId);
        if (myActive) setMyOrder(myActive);
        
        // Play Sound for New Order (Admin Only)
  ...
  const placeOrder = (method) => {
    ...
    // WHATSAPP URL GENERATION (FIXED)
    if (method === 'WA') {
        const itemsList = cart.map(i => `${i.name.en} (x1)`).join(', ');
        const text = `*New Order: ${sessionOrderId}*\nName: ${userDetails.name}\nItems: ${itemsList}\nTotal: ₹${cart.reduce((a,b)=>a+b.price,0)}`;
        window.open(`https://wa.me/919877474778?text=${encodeURIComponent(text)}`, '_blank');
    }
    ...
    const currentOrders = JSON.parse(localStorage.getItem('ft_v14_orders') || '[]');
    localStorage.setItem('ft_v14_orders', JSON.stringify([...currentOrders, newOrder]));
  ...
  const updateStatus = (id, status) => {
      ...
      localStorage.setItem('ft_v14_orders', JSON.stringify(updated));
  ...
  return (
    <div className="bg-[#050505] min-h-screen text-white font-sans selection:bg-orange-500/30 overflow-x-hidden">
      <Head><title>Flavour's Town | V14</title></Head>
      ...
      <header className="fixed top-0 w-full z-[100] px-4 py-3 backdrop-blur-xl bg-black/60 border-b border-white/5 flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <div><h1 className="text-xs font-black uppercase tracking-tight">Flavour's Town</h1><p className="text-[9px] text-gray-400">PUNJAB</p></div>
        </div>
        ...
        {isAdmin ? (
            ...
            <div className="animate-in fade-in duration-500">
                ...
                <div className="space-y-3">
                    {liveOrders.length === 0 ? <p className="text-center text-gray-600 py-10">No Active Orders</p> : 
            ...
            {showCheckout && (
                <motion.div initial={{y:'100%'}} animate={{y:0}} exit={{y:'100%'}} className="fixed inset-0 z-[200] bg-[#050505] p-6 pt-12 overflow-y-auto lg:flex lg:items-center lg:justify-center lg:bg-black/90 lg:backdrop-blur-sm">
                    <div className="lg:w-[400px] lg:bg-[#0a0a0a] lg:p-8 lg:rounded-[2.5rem] lg:border lg:border-white/10">
                        ...
                        {/* --- PAYMENT ACTIONS --- */}
                        <div className="space-y-3">
                            <button onClick={() => setShowScanner(true)} className="w-full bg-zinc-800 text-white py-4 rounded-xl font-black uppercase text-xs tracking-widest border border-white/10 flex items-center justify-center gap-2 animate-pulse">📷 SCAN QR & PAY (UPI)</button>
                            ...
                            <button onClick={() => placeOrder('WA')} className="w-full bg-[#25D366] text-white py-4 rounded-xl font-black uppercase text-xs tracking-widest shadow-lg">WHATSAPP ORDER</button>
                        </div>
                    </div>
                </motion.div>
            )}
